...
Trace는 macOS 기본 캡처 경험을 개선하기 위한 캡처 도구다. MVP에서는 macOS 기본 캡처 도구를 사용하고, 자동 저장, 클립보드 복사, 실행 중인 앱으로의 자동 전달, 최근 및 과거 캡처 히스토리 관리를 핵심으로 한다.
...
MVP에서는 AI 기능과 자체 캡처 엔진을 만들지 않는다. 우선 macOS 기본 캡처 도구 기반의 저장, 전달, 히스토리 기능을 안정적으로 구현하고, 이후 AI 기반 자동 파일명 생성, 폴더 분류, OCR 검색과 함께 Trace 자체 캡처 기능을 검토한다.
